  ...
  },
  {
    ...
    repo: "https://github.com/ziziziwon/dream-candy-lab",
  },
      {
        ...
        summary:
          "전세계 아티스트의 내한 공연을 위한 프리미엄 티켓팅 플랫폼. Mist Indigo(#4C4F7A) 브랜드 컬러로 공연장의 암부와 스모크 무드를 담은 디자인, 인터랙티브 SVG 좌석맵(드래그/확대/축소), 실시간 좌석 상태 관리, CAPTCHA 인증 및 선예매 코드 검증, 팝업 기반 결제 플로우, QR 코드 자동 생성 티켓함, 관리자 대시보드(통계·공연·예매 관리)까지 - 실제 서비스 수준의 완전한 티켓팅 솔루션. Firebase Firestore 기반 실시간 데이터베이스, 멜론티켓/Ticketmaster/KOPIS API 연동, Render 서버 배포까지 완전한 풀스택 아키텍처.",
        category: "Web App · Ticketing · Full-Stack",
        year: "2025",
        role: "Product Design · Full-Stack · UX/UI · Backend · API Integration",
        stack: [
          "React 19.1.1",
          "TypeScript 4.9.5",
        ...
        tags: ["Ticketing", "Firebase", "Full-Stack", "Interactive", "Real-time", "SVG", "Admin", "API", "Render"],
        ...
        features: [
          "🏠 Landing/Home: 최신 공연, 공연장별 보기, 카테고리별 필터링, Swiper 슬라이더",
          "🎤 공연 리스트: 필터링, 검색, 정렬, 랭킹 시스템, 무한 스크롤",
          "🎸 공연 상세: 회차 선택, 가격표, 공연장 정보, 카카오맵 API 연동, 공연 일정 캘린더",
          "🔐 인증 예매: CAPTCHA 보안 인증, 선예매 코드 입력 및 검증, 팬클럽/공식 코드 지원, SMS 인증(구현 예정)",
          "🪑 인터랙티브 SVG 좌석맵: 실시간 좌석 상태 확인, 드래그로 지도 이동, 확대/축소 기능, 좌석 클릭으로 선택/해제, 최대 4매 선택 제한, 등급별 색상 구분 (VIP, R, S, A), 선택된 좌석 가격 라벨 표시",
          "🛒 장바구니: 선택 좌석 확인, 수량 조절, 총 가격 계산, Zustand 상태 관리",
          "💳 결제: 예매자 정보 입력 및 검증, 결제수단 선택(카드/계좌이체/가상계좌), 팝업 결제 플로우, 취소/환불 규정 동의",
          "🎫 티켓함: QR 코드 자동 생성, 티켓 다운로드(PDF), 예매 취소, 공연 안내 정보, 티켓 상태 관리",
          "🔧 관리자 대시보드: 실시간 통계(예매 수, 매출, 사용자 수), Chart.js 차트 시각화, 공연 관리(CRUD), 좌석 관리, 예매 관리 및 취소 처리, 이벤트/추첨 관리, 공지사항 관리",
          "🎨 브랜드 디자인: Mist Indigo(#4C4F7A) 메인 컬러, Slate Violet(#7062A6) 서브 톤, Neon Peach(#FF8C55) 액센트, Soft Gray(#F5F5F5) 섹션 배경",
          "🔒 보안: Firebase Authentication, Firestore 보안 규칙, CAPTCHA 인증, 사용자 권한 관리, Admin/User 역할 구분",
          "🌐 외부 API 연동: 멜론티켓 API 프록시 서버, Ticketmaster API, KOPIS API, 카카오맵 API",
          "🚀 배포: 프론트엔드(카페24 서버), 백엔드 API 서버(Render 배포), 환경 변수 기반 설정",
        ],
        highlights: [
          "디자인 철학: '무대의 어둠과 빛을 담은 Mist Indigo 기반에 Neon Peach 조명을 얹은, 희소성 있는 프리미엄 콘서트 티켓팅 플랫폼' - 포멀하지만 감각적, 공연장의 암부 + 스모크 + 네온 잔광 무드",
          "프로젝트 구조: api/(외부 API 연동), components/(Layout/SeatMap/PaymentPopup/Auth/Admin), pages/(Home/Shows/Reserve/MyTickets/Admin/Events), stores/(Zustand 상태관리), hooks/(커스텀 훅), firebase/(설정 및 서비스)",
          "Firebase 아키텍처: shows(공연), venues(공연장), orders(주문), users/{userId}/tickets(티켓), events(이벤트/추첨) 컬렉션 구조, Firestore 보안 규칙으로 사용자 권한 관리",
          "SVG 좌석맵: 실시간 좌석 상태 확인, 드래그로 지도 이동, 확대/축소 기능, 좌석 클릭으로 선택/해제, 선택된 좌석 가격 라벨 표시, 등급별 색상 구분 (VIP, R, S, A)",
          "인증 시스템: CAPTCHA 보안 인증, 선예매 코드 검증, 팬클럽/공식 코드 지원, SMS 인증(구현 예정), Firebase Authentication 통합",
          "결제 플로우: 팝업 기반 결제 프로세스, 다양한 결제수단 지원(카드/계좌이체/가상계좌), 예매자 정보 입력 및 검증, 취소/환불 규정 동의, 결제 완료 후 자동 팝업 닫기",
          "관리자 기능: 실시간 통계(예매 수, 매출, 사용자 수), Chart.js 차트 시각화, 공연 관리(CRUD), 예매 관리 및 취소 처리, 이벤트/추첨 관리, 공지사항 관리, 사용자 권한 기반 접근 제어",
          "백엔드 서버: Node.js 기반 멜론티켓 API 프록시 서버, Render 배포(https://amplify-concert-server-bqur.onrender.com), Health Check 엔드포인트, CORS 설정",
          "성능 최적화: 코드 스플리팅, 이미지 최적화, 지연 로딩(Lazy Loading), 효율적인 상태 관리(Zustand), React 19 최신 기능 활용",
          "타이포그래피: English(General Sans - 모던, 브랜드 느낌), Korean(SUIT/LINE Seed KR - UI/UX 최적화), Display(Neue Haas Grotesk Display - 히어로/아티스트명)",
          "배포 전략: 프론트엔드(카페24 서버 /amplify 경로), 백엔드(Render 무료 플랜), 환경 변수 기반 API URL 설정, .htaccess SPA 라우팅 지원",
        ],
